...
## challenges this week
    - A particular challenge has been handling missing values in the weather variables, particularly cloud_coverage having a notably high missing rate (~43%).
...
- **Method:**  
  ...
  - Tried other interpolation strategies with the `spline` and `polynomial` approach with different `orders` - no change in the correlation matrix
